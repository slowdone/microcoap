...
static int handle_get_well_known_core(const coap_packet_t *inpkt, coap_packet_t *outpkt, coap_rw_buffer_t *scratch)
{
    ...
    return coap_make_response(inpkt->hdr.id, &inpkt->tok, COAP_RSPCODE_CONTENT,
                              (const uint8_t *)rsp, strlen(rsp), COAP_CONTENTTYPE_APPLICATION_LINKFORMAT,
                              outpkt, scratch);
}
...
static int handle_get_light(const coap_packet_t *inpkt, coap_packet_t *outpkt, coap_rw_buffer_t *scratch)
{
    ...
    return coap_make_response(inpkt->hdr.id, &inpkt->tok, COAP_RSPCODE_CONTENT,
                              (const uint8_t *)&light, 1, COAP_CONTENTTYPE_TEXT_PLAIN,
                              outpkt, scratch);
}

static int handle_put_light(const coap_packet_t *inpkt, coap_packet_t *outpkt, coap_rw_buffer_t *scratch)
{
    printf("handle_put_light\n");
    if (inpkt->payload.len == 0) {
        return coap_make_response(inpkt->hdr.id, &inpkt->tok, COAP_RSPCODE_BAD_REQUEST,
                                  NULL, 0, COAP_CONTENTTYPE_TEXT_PLAIN,
                                  outpkt, scratch);
    }
    ...
    return coap_make_response(inpkt->hdr.id, &inpkt->tok, COAP_RSPCODE_CHANGED,
              (const uint8_t *)&light, 1, COAP_CONTENTTYPE_TEXT_PLAIN,
              outpkt, scratch);
}
